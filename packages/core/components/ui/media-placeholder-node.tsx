  ...
  function PlaceholderElement(props: PlateElementProps<TPlaceholderElement>) {
    ...
    const { openFilePicker } = useFilePicker({
      ...
      onFilesSelected: (data: any) => {
        const updatedFiles =
          (data as { plainFiles?: File[] }).plainFiles ??
          (data as { filesContent?: Array<{ file: File }> }).filesContent?.map(
            (f) => f.file,
          ) ??
          [];
        if (updatedFiles.length) {
          ...
          if (restFiles.length > 0) {
            ...
            editor
              ...
              .insert.media(dt.files as FileList);
          }
        }
      },
    });
    ...
    }, [uploadedFile, element.id, editor, api, element]);
    ...
    }, [api, element.id, replaceCurrentPlaceholder]);
